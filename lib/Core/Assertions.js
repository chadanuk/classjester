...
class Assertions {
    ...
    assertTrue(value, errorMessage = null) {
        ...
        throw new AssertionError_1.AssertionError(`${value.toString()} is not equal to expected value: true`);
    }
    ...
    assertCount(expected, value) {
        ...
        this.throwError(`Count of ${JSON.stringify({ value })} (${countableValue.length}) is not equal to expected value: ${expected.toString()}`);
    }
